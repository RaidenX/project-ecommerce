...
class ProductCart extends Component {
  ...
  onCartDel = id => {
    const ask = window.confirm("Apakah anda yakin menghapus produk ini?? :(");

    if (ask === true) {
      const delId = id;

      axios
        .delete(`http://localhost:1806/carts/${delId}`)

        .then(res => {
          this.getCart();
        });
    } else {
      alert("Terima kasih, untuk tetap memilih produk kami :)");
    }
  };
  ...
  onSaveCart = id => {
    axios.get(`http://localhost:1806/carts/${id}`).then(res => {
      ...
      if (user && prodId) {
        ...
        axios
          .put(`http://localhost:1806/carts/${id}`, {
            productId: prodId,
            username: user,
            name,
            qty: eQty,
        ...
        return (
          <tbody>
            <tr key={item.id}>
              ...
              <td className="text-md-center">{item.qty}</td>
              <td>
                <button
                  onClick={() => {
                    this.onCartEdit(item.id);
                  }}
          ...
          <div className="container mt-5">
            <div className=" row d-flex justify-content-center">
              <h1 className="display-4">
                YOU DON'T HAVE ANY ITEM ON CART PLEASE GO TO{" "}
